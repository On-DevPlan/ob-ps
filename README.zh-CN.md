...
- **启动时快照** —— 在命令上勾选「启动时拍双链快照」；该命令启动时，插件会把当前所有未解析双链记录到完善历史树。
- **完善历史树** —— 可拖动、可缩放的画布（**双链树** 按钮）可视化每一次快照。单击节点跳转到源笔记；画布会自动高亮当前打开的笔记。
...
2. **添加命令** —— 设置 → Local Runner → 命令组管理 → **＋ 新建**。填写名称、shell 命令，以及可选的工作目录；侧边栏随即出现对应的快捷启动按钮。
3. **启动与停止** —— 单击快捷启动栏中该命令的按钮。
4. **查看日志** —— 单击「日志」按钮展开终端输出区，再单击进程卡片展开其日志。
5. **检查双链** —— 上半部分列出未解析双链。单击橡皮擦可清除 vault 内全部未解析双链；也可通过命令面板仅扁平化当前笔记的双链。
6. **捕获快照** —— 在命令上勾选「启动时拍双链快照」；下次启动时，未解析双链会被记录到完善历史树。
7. **打开完善历史树** —— 单击「双链树」按钮。拖动平移、滚轮缩放、双击自适应、单击节点跳转到源笔记。
...
- **命令组管理** —— 管理快捷启动命令：名称、命令、工作目录、可见性，以及「启动时拍双链快照」开关。
